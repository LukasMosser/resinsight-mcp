...
(async () => {
  try {
    ...
    for (const item of manifest) {
      ...
      for (const link of await page.locator('article a[href], .md-nav--primary a[href]').evaluateAll(items => items.map(item => item.href))) {
    ...
    const record = {
      ...
      origin, external_requests_blocked: true, blocked_requests: [...new Set(blockedRequests)],
      pages, links, page_errors: errors, visual_review_required: true,
    };
